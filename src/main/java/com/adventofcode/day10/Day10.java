...
import com.adventofcode.day1.Day1;
 ...
 * a half-twist to reverse the order of the marks within it.
 ...
 * the element at the current position. Move the current position forward by that length plus the skip size.
 ...
 * moves past the end of the list, it wraps around to the front. Lengths
 ...
 * arbitrary ASCII strings, and it also ensures that your input lengths are never
 * larger than 255. For example, if you are given 1,2,3, you should convert
 ...
 * 49,44,50,44,51,17,31,73,47,23 (the ASCII codes from the input string combined with the standard
 ...
 * and skip size should be preserved between rounds. For example, if the
 * previous example was your first round, you would start your second round
 ...
 * some order, called the sparse hash. Your next task is to reduce these to a
 ...
 * below, and the XOR operator is ^, you would calculate the first output number like this:
 *
 ...
 */
public class Day10 implements AdventOfCodePuzzle<Integer, String> {

    private final int size;
    private final int[] lengths;

    Day10(int size, int[] lengths) {
        this.size = size;
        this.lengths = lengths;
    }

    Day10(String input) {
       this(256, IntStream.concat(input.chars(), IntStream.of(17, 31, 73, 47, 23)).toArray());
    }
    ...
    public String solvePartTwo() {
        int[] hash = hashKnot(64);

        int[] denseHash = toDenseHash(hash);

        return toHexademicalString(denseHash);
    }

    private int[] hashKnot(int times) {
        int[] hash = range(0, size).toArray();
        ...
        for (int i = 0; i < times; i++) {
            for (int length : lengths) {
                ...
                currentPosition = circularIndexFor(currentPosition + length + skipSize);
                skipSize++;
            }
        }
    ...
    private void reverse(int[] hash, int currentPosition, int length) {
        int[] reversed = new int[length];

        for (int i = 0, j = currentPosition; i < length; i++, j = circularIndexFor(j + 1)) {
            reversed[i] = hash[j];
        }

        for (int i = 0, j = currentPosition; i < length; i++, j = circularIndexFor(j + 1)) {
            hash[j] = reversed[length - i - 1];
        }
    }

    private static int[] toDenseHash(int[] sparseHash) {
        int[] denseHash = new int[sparseHash.length / 16];

        for (int i = 0; i < denseHash.length; i++) {
            int xored = 0;
            for (int j = 0; j < 16; j++) {
                xored = xored ^ sparseHash[i * 16 + j];
            }
            denseHash[i] = xored;
        }
    ...
    private static String toHexademicalString(int[] hash) {
        return stream(hash).mapToObj(Integer::toHexString).map(Day10::withLeadingZero).collect(joining());
    }

    private static String withLeadingZero(String hex) {
        return hex.length() == 1 ? "0" + hex : hex;
    }

    public static String hashKnot(String input) {
        return new Day10(input).solvePartTwo();
    }
}
