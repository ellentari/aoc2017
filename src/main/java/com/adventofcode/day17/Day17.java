 ...
 * it marks as the current position. It then steps forward through the circular buffer
 ...
 * you assume that's what happened; it's spinning significantly faster
 ...
 */
public class Day17 implements AdventOfCodePuzzle<Integer, Integer> {

    private final int steps;
    private final int insertions;

    Day17(int steps, int insertions) {
        this.steps = steps;
    ...
    public Integer solvePartOne() {
        CircularBuffer buffer = doBufferInsertions();

        return buffer.at(buffer.getCurrentPosition() + 1);
    }

    private CircularBuffer doBufferInsertions() {
        CircularBuffer buffer = new CircularBuffer(0, insertions + 1);

        rangeClosed(1, insertions)
                .forEach(i -> buffer.makeStepsAndInsert(steps, i));
