 ...
 * a memory reallocation routine, but it keeps getting stuck in an infinite loop.
 *
 ...
 * and redistributes those blocks among the banks. To do this, it removes all of the
 * blocks from the selected bank, then moves to the next (by index)
 * memory bank and inserts one of the blocks. It continues doing this until
 ...
 * (the fourth bank) and then continuing to the first bank,
 * the second bank, and so on, the 7 blocks are spread out over the memory banks.
 * The fourth, first, and second banks get two blocks each, and the third bank
 ...
 * from a state that has already been seen, how many block redistribution
 ...
 */
public class Day6 implements AdventOfCodePuzzle<Integer, Integer> {

    private final int[] initialMemory;

    Day6(int[] initialMemory) {
        this.initialMemory = Arrays.copyOf(initialMemory, initialMemory.length);
    }

    /**
     * Determines how many redistribution cycles must be completed before
     * a configuration is produced that has been seen before
     *
     * @return number of redistributions before a cycle encountered
     */
    @Override
    public Integer solvePartOne() {
        return findCycle().getBeginsAfterNOperation();
    }

    /**
     * Determines how many cycles are in the infinite loop that arises
     * from the configuration in the puzzle input
     *
     * @return loop length
     */
    @Override
    public Integer solvePartTwo() {
        return findCycle().getLength();
    }

    private Cycle findCycle() {
        CycleFinder cycleFinder = new CycleFinder(initialMemory);

        while (!cycleFinder.foundCycle()) {
            cycleFinder.nextReallocation();
        }

        return cycleFinder.getFoundCycle();
    }
