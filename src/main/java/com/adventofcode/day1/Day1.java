 ...
 * there are only a few minutes until midnight. "We have a big problem," she says; "there must be almost fifty
 * bugs in this system, but nothing else can print The List. Stand in this square, quick!
 ...
 * all digits that match the next digit in the list. The list is circular,
 ...
 * the door isn't yet satisfied, but it did emit a star as encouragement. The instructions change:
 *
 ...
 */
public class Day1 implements AdventOfCodePuzzle<Integer, Integer> {
    ...
    public Integer solvePartOne() {
        return calculateSumOfEqualElementsWithStep(1);
    }

    @Override
    public Integer solvePartTwo() {
        return calculateSumOfEqualElementsWithStep(input.length() / 2);
    }

    private int calculateSumOfEqualElementsWithStep(int step) {
        int result = 0;

        for (int i = 0; i < input.length(); i++) {
            char current = input.charAt(i);
            char next = nextFor(i, step);
    ...
    private char nextFor(int i, int step) {
        return input.charAt((i + step) % input.length());
    }

    private static int digit(char current) {
        return Character.digit(current, 10);
    }
}
